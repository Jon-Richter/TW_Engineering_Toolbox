...
def solve_simple_beam(
    ...
    I_in4: float = 1.0,
    mesh_max_element_length_ft: float = 1.0,
) -> Dict[str, object]:
    if total_length_ft <= 0:
        raise ValueError("total_length_ft must be > 0.")
    supports = sorted({float(x) for x in support_positions_ft})
    if len(supports) < 2:
        raise ValueError("At least two support positions are required.")
    if supports[0] < -1e-9 or supports[-1] > total_length_ft + 1e-9:
        raise ValueError("Support positions must fall within the beam length.")

    stations = sorted({0.0, total_length_ft, *supports})
    spans = []
    for a, b in zip(stations[:-1], stations[1:]):
        if b - a > 1e-9:
            spans.append(Span(length=b - a, E=E_psi, I=I_in4))
    if not spans:
        raise ValueError("Beam spans could not be constructed from the given positions.")

    joints: List[Joint] = []
    for x in stations:
        restraint_v = any(abs(x - s) <= 1e-9 for s in supports)
        joints.append(Joint(restraint_v=restraint_v, restraint_theta=False))
    ...
    if abs(w_kipft) > 1e-9:
        dist_loads.append(
            DistributedLoad(
                x_start=0.0,
                x_end=total_length_ft,
                w_start=w_kipft * 1000.0,
                w_end=w_kipft * 1000.0,
            )
        )

    point_loads = [
        PointLoad(x=float(pl["x_ft"]), P=float(pl["P_kip"]) * 1000.0)
    ...
    reactions_kip = [
        {
            "x_ft": r["x_user"],
            "reaction_kip": r["reaction_user"] / 1000.0,
        }
        for r in res.get("reactions", [])
        if r.get("dof") == "V"
    ]

    diag = res.get("diagrams", {})
    shear_kip = [v / 1000.0 for v in diag.get("shear_user", [])]
    moment_kipft = [m / 1000.0 for m in diag.get("moment_user", [])]
    deflection_in = diag.get("deflection_user", [])

    max_shear_kip = max((abs(v) for v in shear_kip), default=0.0)
    max_moment_kipft = max((abs(m) for m in moment_kipft), default=0.0)

    return {
        "model": model.model_dump(),
        "reactions_kip": reactions_kip,
        "diagrams": {
            "x_ft": diag.get("x_user", []),
            "shear_kip": shear_kip,
            "moment_kipft": moment_kipft,
            "deflection_in": deflection_in,
        },
        "max_shear_kip": max_shear_kip,
        "max_moment_kipft": max_moment_kipft,
    }
